...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-KOT"
  !define PRODUCT_WEB_SITE "https://github.com/kotiacash/electrum-kot"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-kot-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\electrum_kot\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-kot\*.*"
  File "..\..\electrum_kot\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-kot-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-kot-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-kot-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-kot-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-kot-${PRODUCT_VERSION}.exe" 0


  ;Links kotia: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\kotia" "" "URL:kotia Protocol"
  WriteRegStr HKCU "Software\Classes\kotia" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\kotia" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\kotia\shell\open\command" "" "$\"$INSTDIR\electrum-kot-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\kotia"

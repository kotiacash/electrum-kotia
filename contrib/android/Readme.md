...
```
$ adb -d install -r dist/Electrum-KOT-*-arm64-v8a-debug.apk
$ adb shell monkey -p org.electrum_kot.electrum_kot 1
```
...
```
adb logcat | grep -F "`adb shell ps | grep org.electrum_kot.electrum_kot | cut -c14-19`"
```
...
Run electrum with the `-g` switch: `electrum-kot -g qml`
...
```
$ adb shell
adb$ run-as org.electrum_kot.electrum_kot ls /data/data/org.electrum_kot.electrum_kot/files/data
adb$ exit
$ adb exec-out run-as org.electrum_kot.electrum_kot cat /data/data/org.electrum_kot.electrum_kot/files/data/wallets/my_wallet > my_wallet
```
To push a file:
```
...
adb$ run-as org.electrum_kot.testnet.electrum_kot cp /data/local/tmp/my_wallet /data/data/org.electrum_kot.testnet.electrum_kot/files/data/testnet/wallets/

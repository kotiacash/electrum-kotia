...
GIT_REPO=https://github.com/kotiacash/electrum-kot
...
(
    ...
    LOCALE="$PROJECT_ROOT/electrum_kot/locale/"
    # we want the binary to have only compiled (.mo) locale files; not source (.po) files
    rm -rf "$LOCALE"
    "$CONTRIB/build_locale.sh" "$CONTRIB/deterministic-build/electrum-kot-locale/locale/" "$LOCALE"
...
cp -f "$DLL_TARGET_DIR"/libsecp256k1.*.dylib "$PROJECT_ROOT/electrum_kot/" || fail "Could not copy libsecp256k1 dylib"
...
cp -f "$DLL_TARGET_DIR/libzbar.0.dylib" "$PROJECT_ROOT/electrum_kot/" || fail "Could not copy ZBar dylib"
...
cp -f "$DLL_TARGET_DIR/libusb-1.0.dylib" "$PROJECT_ROOT/electrum_kot/" || fail "Could not copy libusb dylib"
...
### Kotia: do not notarize, this is done manually
#if [ ! -z "$CODESIGN_CERT" ]; then
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-kot-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-kot-${VERSION}.dmg"

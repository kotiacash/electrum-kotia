...
if ! which ${genisoimage} > /dev/null 2>&1; then
    mkdir -p /tmp/electrum-kot-macos
    cd /tmp/electrum-kot-macos
...
rm -rf /tmp/electrum-kot-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-kot-macos/image/
cp -r $1 /tmp/electrum-kot-macos/image/
...
${genisoimage} \
    ...
    -apple \
    -o Electrum-KOT.dmg \
    /tmp/electrum-kot-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-KOT.dmg electrum-kot-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-KOT.dmg

echo "Done."
sha256sum electrum-kot-$VERSION.dmg

...
from electrum_kot.i18n import _, languages
from electrum_kot.util import FileImportFailed, FileExportFailed, make_aiohttp_session, resource_path
from electrum_kot.util import EventListener, event_listener, get_logger
from electrum_kot.invoices import PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING, PR_UNCONFIRMED, PR_BROADCASTING, PR_BROADCAST
from electrum_kot.logging import Logger
from electrum_kot.qrreader import MissingQrDetectionLib
...
def export_meta_gui(electrum_window: 'ElectrumWindow', title, exporter):
    ...
    filename = getSaveFileName(
        ...
        filename='electrum-kot_{}.json'.format(title),
        filter=filter_,
        config=electrum_window.config,
    )
    if not filename:

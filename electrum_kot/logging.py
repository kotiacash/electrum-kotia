...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_kot."):
...
def _delete_old_logs(path, *, num_files_keep: int):
    files = sorted(list(pathlib.Path(path).glob("electrum_kot_log_*.log")), reverse=True)
...
def _configure_file_logging(log_directory: pathlib.Path, *, num_files_keep: int):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_kot_log_{timestamp}_{PID}.log"
...
electrum_logger = logging.getLogger("electrum_kot")
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_kot."):
        name = name[13:]
...
def configure_logging(config: 'SimpleConfig', *, log_to_file: Optional[bool] = None) -> None:
    ...
    _logger.info(f"Electrum-KOT version: {ELECTRUM_VERSION} - https://kotia.cash - {GIT_REPO_URL}")
